...
context.globalThis = context; context.__profInfo = true;
...
api.state.view = "outline";
api.state.tool = "edit";
assert(api._test.applyStationMeasurement("center", "width", 47.5), "station editor: center width edit failed");
assert(Math.abs(api._test.boardCadWidthAtPos(api.state.board, api.state.board.length / 2) - 47.5) < 0.01, "station editor: center width was not applied to the outline");
assert(api._test.applyStationMeasurement("center", "rocker", 0.4), "station editor: center rocker edit failed");
assert(api._test.applyStationMeasurement("center", "thickness", 6.2), "station editor: center thickness edit failed");
assert(Math.abs(api._test.boardCadRockerAtPos(api.state.board, api.state.board.length / 2) - 0.4) < 0.01, "station editor: center rocker was not applied to the profile");
assert(Math.abs(api._test.boardCadThicknessAtPos(api.state.board, api.state.board.length / 2) - 6.2) < 0.01, "station editor: center thickness was not applied to the profile");
assert(api._test.applyStationMeasurement("center", "width", 20, "inch"), "station editor: inch width edit failed");
assert(Math.abs(api._test.boardCadWidthAtPos(api.state.board, api.state.board.length / 2) - 50.8) < 0.01, "station editor: inch input was not converted to centimeters");
assert(api._test.applyBoardLength(182.88), "station editor: length edit failed");
assert(Math.abs(api.state.board.length - 182.88) < 0.01, "station editor: 6ft length was not applied");
api._test.syncStationEditor();
assert(/\d+(?:\.\d+)? L/.test(getElement("stationEditor").textContent), "station editor: volume in liters is missing beside length");
api.state.board = editBoard;
api._test.keepCurrentBoardAsGhost();
const blankGhostWidth = api.state.ghost.board.outline[1].p.y;
api.state.board.outline[1].p.y += 1;
assert(api.state.ghost.board.outline[1].p.y === blankGhostWidth, "blank ghost: editing the board changed its fixed reference outline");
api.state.board = editBoard;
...
if (sectionEnabled("rocker")) {
  ...
  assert(nearNeutralApexKnots.length === 1, "rocker: neutral apply should not create duplicate center knots");
  ...
  const nearNeutralDeckApexKnots = neutralBoard.deck.filter(knot => Math.abs((Number(knot?.p?.x) || 0) - neutralApexX) < 0.5);
  assert(nearNeutralDeckApexKnots.length === 1, "rocker: neutral deck should not create duplicate center knots");
  trace("rocker:done");
}
