...
function boardCadRawWidthAtPos(board, pos) {
  return boardCadSplineValueAt(board.outline, pos) * 2;
}
...
function bottomFeatureOutlineDisplayRawRange(board, item) {
  const length = Math.max(1, Number(board?.length) || 1);
  const count = Math.max(1, Number(item?.trackCount) || 1);
  const index = clampNumber(Number(item?.trackIndex) || 0, 0, count - 1, 0);
  const rawStart = length * ((count - index - 1) / count);
  const rawEnd = length * ((count - index) / count);
  return {
    start: Math.min(rawStart, rawEnd),
    end: Math.max(rawStart, rawEnd),
    peak: (rawStart + rawEnd) * 0.5
  };
}
...
function bezierSegmentFromCurveInterval(curve, t0, t1, shiftX = 0) {
  const span = t1 - t0;
  const p0 = curvePointAt(curve, t0);
  const p3 = curvePointAt(curve, t1);
  const d0 = curveDerivativeAt(curve, t0);
  const d1 = curveDerivativeAt(curve, t1);
  return {
    start: { x: p0.x - shiftX, y: p0.y },
    c1: { x: p0.x + (d0.x * span / 3) - shiftX, y: p0.y + (d0.y * span / 3) },
    c2: { x: p3.x - (d1.x * span / 3) - shiftX, y: p3.y - (d1.y * span / 3) },
    end: { x: p3.x - shiftX, y: p3.y }
  };
}
...
function mirrorOutlinePath(points) {
...
function boardCadTailPlanform(board, segments = getSegments()) {
  ...
  const noseBoard = {
    ...
    tailLength: nose.length,
    tailDepth: 0,
    tailShoulderPos: nose.shoulderPos,
    tailShoulderScale: nose.shoulderScale,
    tailRailBlend: nose.railBlend,
    tailLinearization: nose.linearization,
  ...
  const positiveSpline = reverseSplineAcrossLength(reversedPlanform.positiveSpline, finalLength);
  const mirroredSpline = mirrorSplineYReverse(positiveSpline);
  const positive = dedupeConsecutivePoints(flattenSpline(positiveSpline, Math.max(8, segments)));
  const result = {
    ...base,
    active: base.active || nose.active,
    nose: {
      ...nose,
      tail: reversedPlanform.tail,
      sourceLength: baseLength,
...
function drawSectionsGrid(board, rect) {
  const renderBoard = boardWithPendingBottomFeaturePreview(board);
  const sections = board.sections.filter(section => section.spline.length);
  ...
  sections.forEach((section, index) => {
    ...
    const displaySpline = applyBottomFeaturesToSectionKnots(section.spline, renderBoard, section.position);
    const pts = flattenSpline(displaySpline);
    const mirror = pts.slice().reverse().map(p => ({ x: -p.x, y: p.y }));
    const active = board.sections.indexOf(section) === state.currentSectionIndex;
    drawPointSet(mirror.concat(pts), bounds, {
      stroke: active ? "#ff6b6b" : "#a1a1aa",
      fill: active ? "rgba(255,107,107,.08)" : "rgba(161,161,170,.06)"
    });
    if (active) {
      ctx.strokeStyle = "#ff6b6b";
      ctx.lineWidth = 1.5;
      ctx.strokeRect(bounds.left - 8, bounds.top - 18, bounds.width + 16, bounds.height + 32);
    }
    label(`${section.position.toFixed(1)}`, bounds.left, bounds.top - 8, active ? "#ff6b6b" : "#a1a1aa");
  });
}
...
function averageFaceHeight(points, board) {
  if (!Array.isArray(points) || !points.length) return 0;
  return points.reduce((sum, point) => sum + rotateBoardPoint(point, board).z, 0) / points.length;
}
...
function trimHalfSplineToX(knots, endX) {
  if (!Array.isArray(knots) || knots.length < 2) return boardCadCloneKnots(knots || []);
  const targetX = Math.max(0, Number(endX) || 0);
  const maxX = boardCadSplineMaxX(knots);
  if (targetX >= maxX - 1e-9) return boardCadCloneKnots(knots);
  if (targetX <= knots[0].p.x + 1e-9) {
    const start = cloneKnot(knots[0]);
    start.prev = { ...start.p };
    start.next = { ...start.p };
    return [start];
  }
  const reversed = reverseSpline(boardCadCloneKnots(knots));
  const trimmed = trimHalfSplineFromX(reversed, targetX);
  return reverseSpline(trimmed);
}
...
function boardCadSurfaceProfileAt(board, x, surface) {
  if (!board || board.sections.length < 3) return [];
  const { rawX, displayX } = boardCadSampleXPair(board, x);
  const key = `${state.crossSectionInterpolation}:${surface}:${rawX.toFixed(4)}:${displayX.toFixed(4)}`;
  const cache = crossSectionCacheMap("surface", board);
  ...
  const section = flattenSpline(knots);
  if (!section.length) {
    cache.set(key, []);
    return [];
  }
  const result = surfaceProfile(section, surface);
...
function boardCadSurfaceSectionSampleAt(knots, surface, targetY) {
  ...
  const x = Math.max(0, Math.abs(Number(targetY) || 0));
...
function pointAtPolylineX(points, x) {
  if (!points.length) return null;
...
function boardCadSurfacePointByAngleRange(board, x, fraction, minAngle, maxAngle) {
  ...
  if (state.crossSectionInterpolation === "controlpoint") {
    const knots = boardCadInterpolatedDisplayCrossSectionKnots(board, displayX);
    const point = boardCadPointInAngleRange(knots, fraction, minAngle, maxAngle);
    return point ? { x: displayX, y: point.x, z: point.y + boardCadRockerAtPos(board, rawX) } : null;
  }

  const c1Index = boardCadPreviousCrossSectionIndex(board, rawX);
  const c2Index = boardCadNextCrossSectionIndex(board, rawX);
  if (c1Index < 0 || c2Index < 0) return null;
  const c1 = board.sections[c1Index];
  const c2 = board.sections[c2Index];
  const targetWidth = Math.max(0.5, boardCadDisplayWidthAtPos(board, displayX));
  const targetThickness = Math.max(0.5, boardCadThicknessAtPos(board, rawX));
  const c1Knots = boardCadCrossSectionScaleTo(boardCadCloneKnots(c1.spline), targetThickness, targetWidth);
  const c2Knots = boardCadCrossSectionScaleTo(boardCadCloneKnots(c2.spline), targetThickness, targetWidth);
  const p1 = boardCadPointInAngleRange(c1Knots, fraction, minAngle, maxAngle);
  const p2 = boardCadPointInAngleRange(c2Knots, fraction, minAngle, maxAngle);
  if (!p1 || !p2) return null;
  const pos1 = boardCadPreviousCrossSectionPos(board, rawX);
  const pos2 = boardCadNextCrossSectionPos(board, rawX);
  const t = Math.abs(pos2 - pos1) > 1e-9 ? clamp01((rawX - pos1) / (pos2 - pos1)) : 0;
  return {
    x: displayX,
    y: lerp(p1.x, p2.x, t),
    z: lerp(p1.y, p2.y, t) + boardCadRockerAtPos(board, rawX)
  };
}
...
function shapeExplicitVeeSegment(knots, anchorIndex, railIndex, edgeStrength = 0) {
  if (!Array.isArray(knots) || anchorIndex <= 0 || railIndex <= anchorIndex || railIndex >= knots.length) return;
  const stringer = knots[0];
  const anchor = knots[anchorIndex];
  if (!stringer?.p || !anchor?.p) return;
  const dx1 = Math.max(0.05, anchor.p.x - stringer.p.x);
  const slope1 = (anchor.p.y - stringer.p.y) / dx1;
  const handle1 = dx1 * 0.42;
  stringer.prev = { ...stringer.p };
  stringer.next = {
    x: stringer.p.x + handle1,
    y: stringer.p.y + (slope1 * handle1)
  };
  anchor.prev = {
    x: anchor.p.x - handle1,
    y: anchor.p.y - (slope1 * handle1)
  };
  if (edgeStrength >= 0.1) applyKnotEdgeStyle(anchor, "prev", edgeStrength);
  else anchor.continuous = false;
}
...
function shortboardTailHardEdgeTemplateSpline() {
  const cacheKey = "rail-template-spline:shortboard-tail-hard-edge";
  if (railTemplateCache.has(cacheKey)) return railTemplateCache.get(cacheKey);
  const board = sampleBoardFromEmbeddedData("Shortboard.brd");
  const sections = Array.isArray(board?.sections)
    ? board.sections.slice().sort((a, b) => (Number(a.position) || 0) - (Number(b.position) || 0))
    : [];
  const section = sections.length ? sections[0] : null;
  const template = section?.spline
    ? normalizedTemplateSplineFromSectionSpline(section.spline)
    : null;
  railTemplateCache.set(cacheKey, template || null);
...
function splineFromRailProfilePoints(points) {
  if (!Array.isArray(points) || points.length < 2) return [];
  const railIndex = points.reduce((best, point, index) => point.x > points[best].x ? index : best, 0);
  const knots = points.map((point, index) => {
    const prevPoint = points[Math.max(0, index - 1)];
    const nextPoint = points[Math.min(points.length - 1, index + 1)];
    const prevLen = Math.hypot(point.x - prevPoint.x, point.y - prevPoint.y);
    const nextLen = Math.hypot(nextPoint.x - point.x, nextPoint.y - point.y);
    let prev = { x: point.x, y: point.y };
    let next = { x: point.x, y: point.y };
    if (index === 0) {
      next = { x: point.x + (nextLen * 0.42), y: point.y };
    } else if (index === points.length - 1) {
      prev = { x: point.x + (prevLen * 0.42), y: point.y };
    } else if (index === railIndex) {
      const handleLen = Math.min(prevLen, nextLen) * 0.38;
      prev = { x: point.x, y: point.y - handleLen };
      next = { x: point.x, y: point.y + handleLen };
    } else {
      ...
      const prevHandleLen = prevLen * 0.34;
      const nextHandleLen = nextLen * 0.34;
      prev = { x: point.x - (ux * prevHandleLen), y: point.y - (uy * prevHandleLen) };
      next = { x: point.x + (ux * nextHandleLen), y: point.y + (uy * nextHandleLen) };
    }
    return {
      p: { x: point.x, y: point.y },
      prev,
  ...
  return sanitizeRailSplineKnots(knots);
}
...
function railBranchXAtY(points, branch, y) {
  if (!Array.isArray(points) || points.length < 2) return null;
  const railIndex = points.reduce((best, point, index) => point.x > points[best].x ? index : best, 0);
  const branchPoints = branch === "upper"
    ? points.slice(railIndex)
    : points.slice(0, railIndex + 1);
  if (branchPoints.length < 2) return null;
  const targetY = clampNumber(y, Math.min(branchPoints[0].y, branchPoints[branchPoints.length - 1].y), Math.max(branchPoints[0].y, branchPoints[branchPoints.length - 1].y), y);
  for (let i = 0; i < branchPoints.length - 1; i += 1) {
    const a = branchPoints[i];
    const b = branchPoints[i + 1];
    const minY = Math.min(a.y, b.y);
    const maxY = Math.max(a.y, b.y);
    if (targetY < minY - 1e-9 || targetY > maxY + 1e-9) continue;
    const dy = b.y - a.y;
    if (Math.abs(dy) <= 1e-9) return Math.max(a.x, b.x);
    const t = (targetY - a.y) / dy;
    return lerp(a.x, b.x, t);
  }
  return branch === "upper" ? branchPoints[branchPoints.length - 1].x : branchPoints[0].x;
}
...
function currentGuideTargetLabel() {
  const value = currentGuideTargetValue();
  if (value === "bottom") return t("profile");
  if (value === "deck") return t("deck_label");
  if (value === "section") return t("current_cross_section_short");
  return t("outline");
}
...
function getPanelPointForSelection() {
  if (state.selection.which === 0) return inputPoint(els.cpEndX, els.cpEndY);
  if (state.selection.which === 1) return inputPoint(els.cpPrevX, els.cpPrevY);
  return inputPoint(els.cpNextX, els.cpNextY);
}
...
function outlineHalfWidthAt(points, x) {
  if (!points.length) return 0;
  if (x <= points[0].x) return Math.max(0, Math.abs(points[0].y));
  for (let i = 1; i < points.length; i++) {
    const a = points[i - 1];
    const b = points[i];
    if (x <= b.x) {
      const t = (x - a.x) / Math.max(1e-6, b.x - a.x);
      return Math.max(0, Math.abs(lerp(a.y, b.y, t)));
    }
  }
  return Math.max(0, Math.abs(points[points.length - 1].y));
}
...
function tangentAtPolylineFraction(points, fraction, lengths = null) {
...
function registerBottomFeatureOverlayHandle(handle) {
  const id = `bfh-${bottomFeatureOverlayHandleId++}`;
  bottomFeatureOverlayHandleMap.set(id, handle);
  return id;
}
